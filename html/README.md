...
```tex
  ...
  好处：
	  1. 跨平台
	  	  例如：比如你开发了一款HTML5的游戏，你可以很轻易地移植到UC的开放平台、Opera的游戏中心、Facebook应用平台，甚至可以通过封装的技术发放到App Store或Google Play上
  缺点：
...
nav {
  ...
  display: block;  // 这个也必须加,否则就在IE中显示的行内标签
}
</style>
<nav>自定义标签</nav>
<script>
  document.createElement("nav")   // 这是第一种方案
</script>

<script src="html5shiv.min.js"></script>  // 这是第二种方案，使用插件
